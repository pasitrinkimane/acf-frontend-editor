...
class Acf_Front_End_Editor_Public
{
    ...
    public function register_filters()
    {
        if (!is_customize_preview() && is_user_logged_in() && !is_admin() && $this->user_has_capabilities(get_option($this->option_name . '_capabilities'))):
             ...
             */
            add_filter('acf/load_value/type=qtranslate_text', array ($this, 'acf_targeter'), 10, 3);
            add_filter('acf/format_value/type=qtranslate_textarea', array ($this, 'my_acf_format_value'), 10, 3);
            add_filter('acf/load_value/type=qtranslate_textarea', array ($this, 'acf_targeter'), 10, 3);
            add_filter('acf/format_value/type=qtranslate_textarea', array ($this, 'my_acf_format_value'), 10, 3);
            add_filter('acf/load_value/type=qtranslate_wysiwyg', array ($this, 'acf_wysiwyg_targeter'), 10, 3);
            add_filter('acf/format_value/type=qtranslate_wysiwyg', array ($this, 'my_acf_format_value'), 10, 3);
            ...
            if (is_user_logged_in()):
                ...
                foreach ($textArr as $arr):
                    ...
                    if (function_exists('qtrans_getLanguage')) {
                        global $wpdb;
                        $mid = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->postmeta WHERE post_id = %d AND meta_key = %s", $postid, $name));
                        if (isset($mid[0])) {
                            $fieldRawValue = $mid[0]->meta_value;

                            $blocks = qtranxf_get_language_blocks($fieldRawValue);
                            $texts = qtranxf_split_blocks($blocks);
                            $texts[qtranxf_getLanguage()] = $text;
